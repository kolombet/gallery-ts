...
function getImageTransform(activeItemIdx: number, idx: number) {
  const sideImageOffset = window.innerWidth > 768 ? '4rem' : '1rem';

  if (idx > 0) {
    return `translateX(calc(${-activeItemIdx * 100}% + ${sideImageOffset} + 50vw - 50% )) scaleY(0.83)`;
  } else if (!idx) {
...
class ExtendedCarousel extends React.Component<PropsType, {}> {
  ...
  // eslint-disable-next-line
  slide = (e: any, direction: string) => {
    const { items, activeItemIdx } = this.state;
    const idxDelta = direction === 'left' ? 1 : -1;
    const newActiveItemIdx = activeItemIdx - idxDelta < 0 ? items.length - 1 : (activeItemIdx - idxDelta) % items.length;
  ...
  render() {
    ...
    return (
      <Carousel onClick={this.onCarouselClick}>
        <ItemsWrapper>
          {items.map(({ idx, image }) =>
            <Item key={image.id} style={this.getItemTransformation(idx)} src={image.image} onClick={(e: any) => this.onItemClick(e, idx)} />
          )}
        </ItemsWrapper>
        <Counter>
          {`${(this.state.activeItemIdx + 1) % (this.state.items.length + 1)}/${this.state.items.length} Главный Фасад`}
        </Counter>
      </Carousel>
    );
  }
